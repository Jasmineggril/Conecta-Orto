...
function camels(rows: Record<string, unknown>[]) {
  ...
  try {
    ...
    if (method === "GET" && path === "/admin/stats") {
      ...
      const countMap = Object.fromEntries(
        (counts as { minicourse_id: number; count: number }[]).map((r) => [r.minicourse_id, r.count])
      );
      const total = (totalRow as { count: number }).count;
      const confirmed = (confirmedRow as { count: number }).count;

      return res.json({
        totalRegistrants: total,
        confirmed,
        pending: total - confirmed,
        speakerCount: (speakerRow as { count: number }).count,
        certCount: (certRow as { count: number }).count,
        minicourseEnrollments: (courses as { id: number; title: string; max_capacity: number; active: boolean }[]).map((c) => ({
          id: c.id,
          title: c.title,
    ...
    if (method === "GET" && path === "/admin/registrants") {
      ...
      const [registrants, enrollments] = await Promise.all([
        ...
        query(`SELECT e.registration_id, m.id AS minicourse_id, m.title
               FROM enrollments e JOIN minicourses m ON e.minicourse_id = m.id`),
      ]);

      const enrollMap: Record<number, { id: number; title: string }[]> = {};
      for (const e of enrollments as { registration_id: number; minicourse_id: number; title: string }[]) {
      ...
      return res.json(camels(registrants as Record<string, unknown>[]).map((r) => ({
        ...r,
        minicourses: enrollMap[r.id as number] ?? [],
      })));
    }
    ...
    // ── Admin: update registration ───────────────────────────────────────────
    {
      ...
      if (method === "PUT" && p) {
        ...
        const { status, emailConfirmed, eventPresence, certificateReleased } = body ?? {} as Record<string, unknown>;
        await query(
          `UPDATE registrations SET status=$1, email_confirmed=$2, event_presence=$3, certificate_released=$4, updated_at=now() WHERE id=$5`,
          [status, emailConfirmed, eventPresence, certificateReleased, id]
        );
        const [row] = await query("SELECT * FROM registrations WHERE id=$1", [id]);
        return res.json(camel(row as Record<string, unknown>));
      }
    }
    ...
    if (method === "POST" && path === "/registrations") {
      const { name, email, phone, city, profession } = body ?? {} as Record<string, unknown>;
      ...
      const [reg] = await query(
        `INSERT INTO registrations (name, email, phone, city, profession)
         VALUES ($1,$2,$3,$4,$5) RETURNING *`,
        [name, email, phone, city, profession]
      );

      // Create confirmation token (fire-and-forget)
      const token = crypto.randomBytes(32).toString("hex");
      const expiresAt = new Date(Date.now() + 48 * 60 * 60 * 1000);
      query(
        "INSERT INTO confirmation_tokens (registration_id, token, expires_at) VALUES ($1,$2,$3)",
        [(reg as { id: number }).id, token, expiresAt]
      ).catch(console.error);

      return res.status(201).json(camel(reg as Record<string, unknown>));
    }
    ...
    if (method === "POST" && path === "/registrations/lookup") {
      ...
      const enrollments = await query(
        `SELECT e.minicourse_id AS "minicourseId", m.title
         FROM enrollments e JOIN minicourses m ON e.minicourse_id = m.id
         WHERE e.registration_id = $1`,
        [(reg as { id: number }).id]
      );

      const r = camel(reg as Record<string, unknown>);
      return res.json({ ...r, enrollments });
    }
    ...
    // ── Registrations: by id ─────────────────────────────────────────────────
    {
      ...
      if (method === "GET" && p) {
        ...
        const enrollments = await query(
          `SELECT e.minicourse_id AS "minicourseId" FROM enrollments e WHERE e.registration_id=$1`,
          [(reg as { id: number }).id]
        );
        return res.json({ ...camel(reg as Record<string, unknown>), enrollments });
      }
    }
    ...
    // ── Email confirmation ────────────────────────────────────────────────────
    {
      ...
      if (method === "GET" && p) {
        const [tok] = await query(
          "SELECT * FROM confirmation_tokens WHERE token=$1 AND used_at IS NULL AND expires_at > now()",
          [p.token]
        );
        if (!tok) return res.status(400).json({ error: "Token inválido ou expirado" });
        const t = tok as { registration_id: number; id: number };
        await query("UPDATE registrations SET email_confirmed=true, email_confirmed_at=now() WHERE id=$1", [t.registration_id]);
        await query("UPDATE confirmation_tokens SET used_at=now() WHERE id=$1", [t.id]);
        const [reg] = await query("SELECT name FROM registrations WHERE id=$1", [t.registration_id]);
        return res.json({ success: true, name: (reg as { name: string }).name });
      }
    }

    // ── Minicourses: public list ─────────────────────────────────────────────
    if (method === "GET" && path === "/minicourses") {
      ...
      return res.json(
        (courses as Record<string, unknown>[]).map((c) => ({
          ...
          enrollmentCount: countMap[c.id as number] ?? 0,
        }))
      );
    }

    // ── Minicourses: admin list ──────────────────────────────────────────────
    ...
    if (method === "POST" && path === "/minicourses") {
      ...
      const { title, instructor, description, duration, maxCapacity = 30, type = "pratico",
        ...
        generatesCertificate = true, enrollmentsClosed = false } = body ?? {} as Record<string, unknown>;
      ...
      return res.status(201).json(camel(row as Record<string, unknown>));
    }
    ...
    // ── Minicourses: update ──────────────────────────────────────────────────
    {
      ...
      if (method === "PUT" && p) {
        if (!verifyToken(req)) return res.status(401).json({ error: "Não autorizado" });
        const id = Number(p.id);
        const { title, instructor, description, duration, maxCapacity, type, active, date, time, location, workload, generatesCertificate, enrollmentsClosed } = body ?? {} as Record<string, unknown>;
        const [row] = await query(
          `UPDATE minicourses SET title=$1,instructor=$2,description=$3,duration=$4,max_capacity=$5,type=$6,active=$7,date=$8,time=$9,location=$10,workload=$11,generates_certificate=$12,enrollments_closed=$13
           WHERE id=$14 RETURNING *`,
          [title, instructor, description, duration, maxCapacity, type, active, date, time, location, workload, generatesCertificate, enrollmentsClosed, id]
        );
      ...
      if (method === "GET" && p) {
        ...
        const rows = await query(
          `SELECT e.id AS "enrollmentId", r.id AS "registrationId", r.name, r.email, r.phone,
           ...
           FROM enrollments e JOIN registrations r ON e.registration_id = r.id
           WHERE e.minicourse_id=$1`,
          [Number(p.id)]
        );
    ...
    // ── Minicourses: mark presence ───────────────────────────────────────────
    {
      ...
      if (method === "POST" && p) {
        if (!verifyToken(req)) return res.status(401).json({ error: "Não autorizado" });
        const { present } = body ?? {} as Record<string, unknown>;
        await query(
          "UPDATE enrollments SET minicourse_presence=$1 WHERE minicourse_id=$2 AND registration_id=$3",
          [present, Number(p.id), Number(p.registrationId)]
        );
        return res.json({ success: true });
      }
    }
    ...
    if (method === "POST" && path === "/enrollments") {
      const { registrationId, minicourseId } = body ?? {} as Record<string, unknown>;
      ...
      const c = course as { id: number; max_capacity: number; enrollments_closed: boolean };
      if (c.enrollments_closed || (count as number) >= c.max_capacity) {
      ...
      try {
        ...
        return res.status(201).json(camel(enrollment as Record<string, unknown>));
      } catch {
    ...
    if (method === "GET" && path === "/speakers") {
      const rows = await query("SELECT * FROM speakers WHERE active=true ORDER BY display_order, id");
      return res.json(camels(rows as Record<string, unknown>[]));
    }

    if (method === "GET" && path === "/speakers/all") {
      if (!verifyToken(req)) return res.status(401).json({ error: "Não autorizado" });
      const rows = await query("SELECT * FROM speakers ORDER BY display_order, id");
      return res.json(camels(rows as Record<string, unknown>[]));
    }

    if (method === "POST" && path === "/speakers") {
      if (!verifyToken(req)) return res.status(401).json({ error: "Não autorizado" });
      const { name, title = "", institution = "", bio = "", talkTopic = "", talkDate = "", talkTime = "", linkedinUrl = null, photoUrl = null, displayOrder = 0, active = true } = body ?? {} as Record<string, unknown>;
      const [row] = await query(
        `INSERT INTO speakers (name,title,institution,bio,talk_topic,talk_date,talk_time,linkedin_url,photo_url,display_order,active)
...

    {
      ...
      if (method === "PUT" && p) {
        ...
        const { name, title = "", institution = "", bio = "", talkTopic = "", talkDate = "", talkTime = "", linkedinUrl = null, photoUrl = null, displayOrder = 0, active = true } = body ?? {} as Record<string, unknown>;
        const [row] = await query(
          `UPDATE speakers SET name=$1,title=$2,institution=$3,bio=$4,talk_topic=$5,talk_date=$6,talk_time=$7,linkedin_url=$8,photo_url=$9,display_order=$10,active=$11
           WHERE id=$12 RETURNING *`,
          [name, title, institution, bio, talkTopic, talkDate, talkTime, linkedinUrl, photoUrl, displayOrder, active, id]
        );
        if (!row) return res.status(404).json({ error: "Não encontrado" });
        return res.json(camel(row as Record<string, unknown>));
      }
    ...
    if (method === "GET" && path === "/sponsors") {
      const rows = await query("SELECT * FROM sponsors WHERE active=true ORDER BY display_order, id");
      return res.json(camels(rows as Record<string, unknown>[]));
    }

    if (method === "GET" && path === "/sponsors/all") {
      if (!verifyToken(req)) return res.status(401).json({ error: "Não autorizado" });
      const rows = await query("SELECT * FROM sponsors ORDER BY display_order, id");
      return res.json(camels(rows as Record<string, unknown>[]));
    }

    if (method === "POST" && path === "/sponsors") {
      if (!verifyToken(req)) return res.status(401).json({ error: "Não autorizado" });
      const { name, logoUrl = "", websiteUrl = "", category = "apoio", displayOrder = 0, active = true } = body ?? {} as Record<string, unknown>;
      const [row] = await query(
...

    {
      ...
      if (method === "PUT" && p) {
        ...
        const [row] = await query(
          ...
          [name, logoUrl, websiteUrl, category, displayOrder, active, id]
        );
        if (!row) return res.status(404).json({ error: "Não encontrado" });
        return res.json(camel(row as Record<string, unknown>));
      }
    ...
    if (method === "GET" && path === "/gallery") {
      const rows = await query("SELECT * FROM gallery_items WHERE active=true ORDER BY display_order, id");
      return res.json(camels(rows as Record<string, unknown>[]));
    }

    if (method === "GET" && path === "/gallery/all") {
      if (!verifyToken(req)) return res.status(401).json({ error: "Não autorizado" });
      const rows = await query("SELECT * FROM gallery_items ORDER BY display_order, id");
      return res.json(camels(rows as Record<string, unknown>[]));
    }

    if (method === "POST" && path === "/gallery") {
      if (!verifyToken(req)) return res.status(401).json({ error: "Não autorizado" });
      const { title = "", caption = "", altText = "", imageUrl, category = "geral", displayOrder = 0, active = true } = body ?? {} as Record<string, unknown>;
...

    {
      ...
      if (method === "PUT" && p) {
        ...
        const [row] = await query(
          ...
          [title, caption, altText, imageUrl, category, displayOrder, active, id]
        );
        if (!row) return res.status(404).json({ error: "Não encontrado" });
        return res.json(camel(row as Record<string, unknown>));
      }
    ...
    if (method === "GET" && path === "/homepage") {
      ...
      for (const r of rows as { key: string; value: string }[]) content[r.key] = r.value;
    ...
    if (method === "PUT" && path === "/homepage") {
      ...
      for (const [key, value] of Object.entries(updates ?? {})) {
        await query(
          `INSERT INTO homepage_content (key, value) VALUES ($1,$2)
    ...
    // ── Certificates: validate ────────────────────────────────────────────────
    {
      ...
      if (method === "GET" && p) {
        const [cert] = await query(
          `SELECT c.id, c.type, c.validation_code, c.workload, c.status, c.issued_at,
                  r.name, m.title AS minicourse_title
           FROM certificates c
           JOIN registrations r ON c.registration_id = r.id
           LEFT JOIN minicourses m ON c.minicourse_id = m.id
           WHERE c.validation_code=$1`,
          [p.code.toUpperCase()]
        );
        if (!cert) return res.status(404).json({ valid: false, error: "Certificado não encontrado" });
        const c = cert as { status: string; name: string; type: string; minicourse_title: string; workload: string; issued_at: Date; validation_code: string };
        return res.json({
          valid: c.status === "ativo",
          name: c.name,
          type: c.type,
          title: c.type === "event" ? "Conecta Orto 2026" : (c.minicourse_title ?? ""),
          workload: c.workload,
